...
sademote = regex_or(
        # Standard version  :) :( :] :D :P
        "(?:>|&gt;)?" + regex_or(tongue+r"(?=\W|$|RT|rt|Rt)", sadMouths),

        regex_or("(?<=(?: ))", "(?<=(?:^))") + regex_or(sadMouths,) + noseArea + "(?:<|&lt;)?",

)

otheremote = regex_or(
        # Standard version  :) :( :] :D :P
        "(?:>|&gt;)?" + regex_or(noseArea,"[Oo]") + regex_or(tongue+r"(?=\W|$|RT|rt|Rt)", otherMouths+r"(?=\W|$|RT|rt|Rt)"),

        # reversed version (: D:  use positive lookbehind to remove "(word):"
        # because eyes on the right side is more ambiguous with the standard usage of : ;
        regex_or("(?<=(?: ))", "(?<=(?:^))") + regex_or(otherMouths) + noseArea + "(?:<|&lt;)?",
        oOEmote
)
...
Protected2  = re.compile(
    regex_or(
        Hearts,
        emoticon,
        decorations), re.UNICODE)

happy_emote  = re.compile(regex_or(happyemote), re.UNICODE)

sad_emote  = re.compile(regex_or(sademote), re.UNICODE)

other_emote  = re.compile(regex_or(otheremote), re.UNICODE)

deco_emote = re.compile(regex_or(decorations), re.UNICODE)

heart_emote = re.compile(regex_or(Hearts), re.UNICODE)

url_regex = re.compile(regex_or(url), re.UNICODE)

email_regex = re.compile(regex_or(Email), re.UNICODE)
